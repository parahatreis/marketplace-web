  ...
  useEffect(() => {
    if (product.stocks) {
      if (product.stocks[0].sizeTypeId) {
        ...
        const findStock = product.stocks.find(
          (stock) => stock.sizeName.size_name_id === sizeId
        );
            ...
            <div className="size">
              ...
              <ul className="size__list">
                {product.stocks.map((size, index) => (
                  <li
                    className={`${
                      size.sizeName.size_name_id === currentSize ? "active" : ""
                    }`}
                    onClick={() => onSizeChange(size.sizeName.size_name_id)}
                    key={index}
                  >
